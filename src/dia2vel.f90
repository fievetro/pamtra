...
!       velSpec = Av * diaSpec**Bv !2.23
  ...
  subroutine dia2vel_atlas &
    ...
    velSpec)            ! out
      ...
      real(kind=dbl) :: D_lim !added by Margaux
      real(kind=dbl), dimension(nDia) :: D !added by Margaux
      integer(kind=long), intent(out) :: errorstatus
      integer(kind=long) :: err = 0
      character(len=80) :: msg
      ...
      ! Margaux's try with PO's formula (for original pamtra, comment the following lines and uncomment the previous one)
      D_lim = 0.1d0 * 1.0d-3
      D(:) = (6.0d0*mass(:)/(pi*rho_water))**(1.0d0/3.0d0)
      where (D < D_lim)
        velSpec = 24.0d0 * D**2
      elsewhere
        velSpec = fallvel_A - fallvel_B * DEXP(-fallvel_C * D)
      end where
